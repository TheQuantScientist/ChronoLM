...
# Models used in this legacy comparison plot.
MODEL_LIST = [
...
print(f"[*] Drawing a 10-patient grid. Lookback: {LOOKBACK}")
...
for i, pid in enumerate(patient_ids):
    ...
    if len(all_times) > 15:
        ax.set_xticks(range(0, len(all_times), 2))
    else:
...
print(f"\n[DONE] Saved comparison grid: {output_name}")
plt.show()
